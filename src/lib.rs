...
pub const CURRENT_BLOCK_HEIGHT: u64 = 951_048;
...
pub fn validate_block_height(height: i64) -> (bool, String) {
    ...
    let (is_valid, message) = if height < 0 {
        (false, String::from("Block height is negative"))
...
pub fn halving_schedule(blocks: &[u64]) -> HashMap<u64, u64> {
    ...
    let base_reward: u64 = 50 * 100_000_000; // 50 BTC in satoshis
...
pub fn create_utxo(
    ...
    utxo_map.insert(String::from("txid"), String::from(txid));

    utxo_map.insert(String::from("vout"), vout.to_string());
